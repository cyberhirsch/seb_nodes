/* Panorama editor vendored into seb_nodes.
   Derived from ComfyUI-Panorama-Stickers - MIT License, (c) 2026 nomadoor.
   Full licence text in LICENSE-panorama-stickers.txt.
   Modified: bound to Read360Seb, renamed extension, preview attach disabled. */
...
function wx({
  ...
  return {
    name: "Seb.PanoEditor.Vendored",
    beforeRegisterNodeDef(f, h) {
      const g = String((h == null ? void 0 : h.name) || "");
      (g === "Read360Seb") && mh(f, {
        app: e,
        matchType: "PanoramaCutout",
        buttonText: "Open Pano Editor",
        openEditor: t,
        attachStickers: n,
        /* seb_nodes: their cutout surface stacks a second ERP preview onto the node
           face, next to our framing preview, and hides the editor button behind it.
           No-op it: Hu() still adds "Open Pano Editor", and with no
           __panoCutoutNodeSurface the Ps() call leaves that button visible. */
        attachCutout: () => {},
        enableStickersPreview: !1
      }), (false && g === "PanoramaStickers") && mh(f, {
        app: e,
...
function qx(e, t = []) {
  /* seb_nodes: Read360Seb has no erp_image input, so their per-input loop and the
     "background" gate both skip it. Resolve our panorama explicitly: the node's
     own execution output first (base+layer composite), then the image widget. */
  try {
    const __cls = String((e && (e.comfyClass || e.type)) || "");
    if (__cls === "Read360Seb") {
      const __app = window.app || {};
      const __no = __app.nodeOutputs || {};
      const __out = __no[e.id] || __no[String(e.id)] || null;
      const __pii = __out && (__out.pano_input_images ||
                              (__out.ui && __out.ui.pano_input_images));
      if (Array.isArray(__pii) && __pii.length) {
        const f = __pii[0];
        const q = "/api/view?filename=" + encodeURIComponent(f.filename || "") +
                  "&type=" + encodeURIComponent(f.type || "temp") +
                  "&subfolder=" + encodeURIComponent(f.subfolder || "");
        return { src: q, sourceType: "sebPanoOutput", inputName: "erp_image" };
      }
      const __w = (e.widgets || []).find(
        (x) => String((x && x.name) || "").toLowerCase() === "image");
      if (__w && __w.value && !String(__w.value).includes("/")) {
        const q = "/api/view?filename=" + encodeURIComponent(String(__w.value)) +
                  "&type=input&subfolder=";
        return { src: q, sourceType: "sebPanoWidget", inputName: "erp_image" };
      }
    }
  } catch (err) { /* fall through to their resolution */ }
  ...
  for (const x of c) {
    ...
    if (H) {
      let se = gr(H.value);
      if (se && !se.includes("/") && !se.includes(":") && (z.comfyClass === "LoadImage" || z.type === "LoadImage" || z.comfyClass === "Read360Seb" || z.type === "Read360Seb") && (se = bn.apiURL(`/view?filename=${encodeURIComponent(se)}&type=input&subfolder=`)), se)
...
function GN(e, t) {
  const n = String((e == null ? void 0 : e.comfyClass) || (e == null ? void 0 : e.type) || (e == null ? void 0 : e.title) || "").trim(), r = {
    ...
    Read360Seb: "Panorama Cutout", PanoramaPreview: "Panorama Preview",
